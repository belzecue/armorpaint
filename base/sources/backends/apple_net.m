...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
	...
	if (dstPath) {
		...
		// todo
		if ([dstPath hasSuffix:@"iris"] || [dstPath hasSuffix:@"llama_metal"]) {
...
void iron_net_request(const char *url_base, const char *url_path, const char *data, int port, int method, iron_https_callback_t callback, void *callbackdata,
	...
	NSString *urlStr =
	    [NSString stringWithFormat:@"https://%@:%d/%@", [NSString stringWithUTF8String:url_base], port, [NSString stringWithUTF8String:url_path]];
