...
void iris_print_blas_profile(void) {
	double total = prof_single_adaln + prof_single_fused_matmul + prof_single_split + prof_single_qknorm_rope + prof_single_attention + prof_single_swiglu +
 ...
 */
static inline void gated_add(float *out, const float *gate, const float *proj, int seq, int hidden) {
	for (int s = 0; s < seq; s++) {
...
static int parse_transformer_config(const char *model_dir, iris_transformer_flux_t *tf) {
	...
	if ((p = strstr(buf, "\"joint_attention_dim\""))) {
		if ((p = strchr(p, ':')))
			joint_attention_dim = atoi(p + 1);
	}
	if ((p = strstr(buf, "\"in_channels\""))) {
		if ((p = strchr(p, ':')))
			in_channels = atoi(p + 1);
	}
...
static void register_q8_ptr(const void *p) {
	if (!p || flux_ptr_is_q8(p))
		return; /* dedup: blocks reload often */
	if (g_q8_nptrs < MAX_Q8_PTRS)
		g_q8_ptrs[g_q8_nptrs++] = p;
#ifdef USE_METAL
...
static int gguf_map_name(const char *d, char *out, size_t outsz, int *slice, int *nslice) {
	int i;
...
#define MAP(PAT, OUT)                                          \
	do {                                                       \
		...
		if (sscanf(d, PAT "%n", &i, &_n) == 1 && _n == dlen) { \
			snprintf(out, outsz, OUT, i);                      \
			return 1;                                          \
		}                                                      \
	} while (0)
#define MAPS(PAT, OUT, SL)                                     \
	do {                                                       \
		int _n = -1;                                           \
		if (sscanf(d, PAT "%n", &i, &_n) == 1 && _n == dlen) { \
			snprintf(out, outsz, OUT, i);                      \
...
#undef MAPS
	...
	/* --- top-level tensors --- */
	if (strcmp(d, "x_embedder.weight") == 0) {
		snprintf(out, outsz, "img_in.weight");
		return 1;
	}
	if (strcmp(d, "context_embedder.weight") == 0) {
		snprintf(out, outsz, "txt_in.weight");
		return 1;
	}
	if (strcmp(d, "time_guidance_embed.timestep_embedder.linear_1.weight") == 0) {
		snprintf(out, outsz, "time_in.in_layer.weight");
		return 1;
	}
	if (strcmp(d, "time_guidance_embed.timestep_embedder.linear_2.weight") == 0) {
		snprintf(out, outsz, "time_in.out_layer.weight");
		return 1;
	}
	if (strcmp(d, "double_stream_modulation_img.linear.weight") == 0) {
		snprintf(out, outsz, "double_stream_modulation_img.lin.weight");
		return 1;
	}
	if (strcmp(d, "double_stream_modulation_txt.linear.weight") == 0) {
		snprintf(out, outsz, "double_stream_modulation_txt.lin.weight");
		return 1;
	}
	if (strcmp(d, "single_stream_modulation.linear.weight") == 0) {
		snprintf(out, outsz, "single_stream_modulation.lin.weight");
		return 1;
	}
	if (strcmp(d, "norm_out.linear.weight") == 0) {
		snprintf(out, outsz, "final_layer.adaLN_modulation.1.weight");
		return 1;
	}
	if (strcmp(d, "proj_out.weight") == 0) {
		snprintf(out, outsz, "final_layer.linear.weight");
		return 1;
	}
...
static int open_transformer_shards(const char *model_dir, safetensors_file_t **files, int max_files) {
	char path[1024];
	int  num_files = 0;
	 ...
	 * fallback for backends without the Q8 path. */
	{
		...
		for (size_t gi = 0; gi < sizeof(gguf_names) / sizeof(gguf_names[0]); gi++) {
			...
			if (iris_verbose)
				fprintf(stderr, "Loading transformer from GGUF: %s%s\n", gguf_name, g_transformer_weights_q8 ? " (Q8_0 quantized)" : "");
			return 1;
		}
	}
				...
				while (p && num_shards < max_files) {
					...
					if (slen > 0 && slen < 256) {
						...
						for (int i = 0; i < num_shards; i++) {
							if (strcmp(shard_names[i], fname) == 0) {
								dup = 1;
								break;
							}
						}
 ...
 * (slice == -1 when none). */
static const char *resolve_tensor_name(const char *name, char *buf, size_t bufsz, int *slice, int *nslice) {
	*slice  = -1;
	*nslice = 1;
...
static uint16_t *mmap_get_bf16(safetensors_file_t **files, int num_files, const char *name) {
	...
	for (int f = 0; f < num_files; f++) {
		...
		if (t && safetensor_is_bf16(t)) {
			...
			if (p && slice >= 0)
				p += (size_t)slice * (safetensor_numel(t) / nslice);
			return p;
		}
...
static int load_double_block_weights(double_block_t *b, safetensors_file_t **files, int num_files, int idx, int h, int mlp, int use_bf16) {
	...
	if (use_bf16)
		b->img_q_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->img_q_weight = mmap_get_f32(files, num_files, name);
	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_k.weight", idx);
	if (use_bf16)
		b->img_k_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->img_k_weight = mmap_get_f32(files, num_files, name);
	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_v.weight", idx);
	if (use_bf16)
		b->img_v_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->img_v_weight = mmap_get_f32(files, num_files, name);

	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_out.0.weight", idx);
	if (use_bf16)
		b->img_proj_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->img_proj_weight = mmap_get_f32(files, num_files, name);
	...
	if (use_bf16)
		b->img_mlp_down_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->img_mlp_down_weight = mmap_get_f32(files, num_files, name);
	...
	if (use_bf16)
		b->txt_q_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->txt_q_weight = mmap_get_f32(files, num_files, name);
	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.add_k_proj.weight", idx);
	if (use_bf16)
		b->txt_k_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->txt_k_weight = mmap_get_f32(files, num_files, name);
	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.add_v_proj.weight", idx);
	if (use_bf16)
		b->txt_v_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->txt_v_weight = mmap_get_f32(files, num_files, name);

	snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_add_out.weight", idx);
	if (use_bf16)
		b->txt_proj_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->txt_proj_weight = mmap_get_f32(files, num_files, name);
	...
	if (use_bf16)
		b->txt_mlp_down_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->txt_mlp_down_weight = mmap_get_f32(files, num_files, name);
...
static void free_double_block_weights(double_block_t *b) {
	...
	b->img_norm_q_weight = NULL;
	free(b->img_norm_k_weight);
	b->img_norm_k_weight = NULL;
	free(b->img_q_weight);
	b->img_q_weight = NULL;
	free(b->img_k_weight);
	b->img_k_weight = NULL;
	...
	b->img_proj_weight = NULL;
	free(b->img_mlp_gate_weight);
	b->img_mlp_gate_weight = NULL;
	free(b->img_mlp_up_weight);
	b->img_mlp_up_weight = NULL;
	free(b->img_mlp_down_weight);
	b->img_mlp_down_weight = NULL;
	...
	b->txt_norm_q_weight = NULL;
	free(b->txt_norm_k_weight);
	b->txt_norm_k_weight = NULL;
	free(b->txt_q_weight);
	b->txt_q_weight = NULL;
	free(b->txt_k_weight);
	b->txt_k_weight = NULL;
	...
	b->txt_proj_weight = NULL;
	free(b->txt_mlp_gate_weight);
	b->txt_mlp_gate_weight = NULL;
	free(b->txt_mlp_up_weight);
	b->txt_mlp_up_weight = NULL;
	free(b->txt_mlp_down_weight);
	b->txt_mlp_down_weight = NULL;
	/* bf16 pointers are direct mmap pointers - just clear, don't free */
...
static int load_single_block_weights(single_block_t *b, safetensors_file_t **files, int num_files, int idx, int h, int mlp, int use_bf16) {
	char name[256];
	(void)h;
	(void)mlp; /* Unused in single block */
	...
	if (use_bf16)
		b->qkv_mlp_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->qkv_mlp_weight = mmap_get_f32(files, num_files, name);
	...
	if (use_bf16)
		b->proj_mlp_weight_bf16 = mmap_get_bf16(files, num_files, name);
	if (!use_bf16)
		b->proj_mlp_weight = mmap_get_f32(files, num_files, name);
...
static void free_single_block_weights(single_block_t *b) {
	...
	b->norm_q_weight = NULL;
	free(b->norm_k_weight);
	b->norm_k_weight = NULL;
	free(b->qkv_mlp_weight);
	b->qkv_mlp_weight = NULL;
	free(b->proj_mlp_weight);
	b->proj_mlp_weight = NULL;
	/* bf16 pointers are direct mmap pointers - just clear, don't free */
...
static void warmup_mmap_bf16_buffers(iris_transformer_flux_t *tf) {
	...
	if (tf->img_in_weight_bf16)
		iris_metal_warmup_bf16_buffer(tf->img_in_weight_bf16, (size_t)tf->latent_channels * h);
	if (tf->txt_in_weight_bf16)
		iris_metal_warmup_bf16_buffer(tf->txt_in_weight_bf16, (size_t)tf->text_dim * h);
	if (tf->final_proj_weight_bf16)
		iris_metal_warmup_bf16_buffer(tf->final_proj_weight_bf16, (size_t)tf->latent_channels * h);
...
static void compute_rope_2d(float *cos_out, float *sin_out, int patch_h, int patch_w, int axis_dim, float theta) {
 ...
 */
static void compute_rope_2d_with_t_offset(float *cos_out, float *sin_out, int patch_h, int patch_w, int axis_dim, float theta, int t_offset) {
...
static void apply_rope_2d(float *x, const float *cos_freq, const float *sin_freq, int seq, int heads, int head_dim, int axis_dim) {
...
static void compute_rope_text(float *cos_out, float *sin_out, int txt_seq, int axis_dim, float theta) {
...
static void get_cached_img_rope(iris_transformer_flux_t *tf, int patch_h, int patch_w, float **cos_out, float **sin_out) {
	int    seq      = patch_h * patch_w;
	int    axis_dim = tf->axis_dim;
	size_t size     = (size_t)seq * axis_dim * 4 * sizeof(float);

	if (tf->cached_img_h == patch_h && tf->cached_img_w == patch_w && tf->cached_img_rope_cos && tf->cached_img_rope_sin) {
		/* Cache hit */
	...
	if (tf->cached_img_rope_cos)
		free(tf->cached_img_rope_cos);
	if (tf->cached_img_rope_sin)
	...
	compute_rope_2d(tf->cached_img_rope_cos, tf->cached_img_rope_sin, patch_h, patch_w, axis_dim, tf->rope_theta);
...
static void get_cached_ref_rope(iris_transformer_flux_t *tf, int patch_h, int patch_w, int t_offset, float **cos_out, float **sin_out) {
	int    seq      = patch_h * patch_w;
	int    axis_dim = tf->axis_dim;
	size_t size     = (size_t)seq * axis_dim * 4 * sizeof(float);

	if (tf->cached_ref_h == patch_h && tf->cached_ref_w == patch_w && tf->cached_ref_t_offset == t_offset && tf->cached_ref_rope_cos &&
	    tf->cached_ref_rope_sin) {
		/* Cache hit */
	...
	/* Cache miss - reallocate if needed */
	if (tf->cached_ref_rope_cos)
		free(tf->cached_ref_rope_cos);
	if (tf->cached_ref_rope_sin)
	...
	compute_rope_2d_with_t_offset(tf->cached_ref_rope_cos, tf->cached_ref_rope_sin, patch_h, patch_w, axis_dim, tf->rope_theta, t_offset);
...
static void get_cached_txt_rope(iris_transformer_flux_t *tf, int txt_seq, float **cos_out, float **sin_out) {
	int    head_dim = tf->head_dim;
	size_t size     = (size_t)txt_seq * head_dim * sizeof(float);

	if (tf->cached_txt_seq == txt_seq && tf->cached_txt_rope_cos && tf->cached_txt_rope_sin) {
		/* Cache hit */
	...
	/* Cache miss - reallocate if needed */
	if (tf->cached_txt_rope_cos)
		free(tf->cached_txt_rope_cos);
	if (tf->cached_txt_rope_sin)
	...
	compute_rope_text(tf->cached_txt_rope_cos, tf->cached_txt_rope_sin, txt_seq, tf->axis_dim, tf->rope_theta);
...
static void get_cached_combined_rope(iris_transformer_flux_t *tf, int img_h, int img_w, int ref_h, int ref_w, int t_offset, float **cos_out, float **sin_out) {
	...
	if (tf->cached_combined_img_h == img_h && tf->cached_combined_img_w == img_w && tf->cached_combined_ref_h == ref_h && tf->cached_combined_ref_w == ref_w &&
	    tf->cached_combined_t_offset == t_offset && tf->cached_combined_rope_cos && tf->cached_combined_rope_sin) {
		/* Cache hit */
	...
	/* Cache miss - reallocate if needed */
	if (tf->cached_combined_rope_cos)
		free(tf->cached_combined_rope_cos);
	if (tf->cached_combined_rope_sin)
...
static void time_embed_forward(float *out, const float *t_sincos, const time_embed_t *te, int hidden, float *work) {
...
static void apply_adaln(float *out, const float *x, const float *shift, const float *scale, int seq, int hidden, float eps) {
...
static void apply_qk_norm(float *q, float *k, const float *q_weight, const float *k_weight, int seq, int heads, int head_dim, float eps) {
	/* RMSNorm per head */
	for (int s = 0; s < seq; s++) {
		for (int h = 0; h < heads; h++) {
			/* Q normalization */
...
static void transpose_shd_to_hsd(float *out, const float *in, int seq, int heads, int head_dim) {
	for (int s = 0; s < seq; s++) {
		for (int h = 0; h < heads; h++) {
...
static void transpose_hsd_to_shd(float *out, const float *in, int seq, int heads, int head_dim) {
	for (int h = 0; h < heads; h++) {
		for (int s = 0; s < seq; s++) {
...
static int ensure_work_buffers(iris_transformer_flux_t *tf, int total_seq) {
	...
	if (!tf->img_hidden || !tf->txt_hidden || !tf->work1 || !tf->work2 || !tf->attn_q_t || !tf->attn_k_t || !tf->attn_v_t || !tf->attn_out_t ||
	    !tf->attn_cat_k || !tf->attn_cat_v || !tf->single_q || !tf->single_k || !tf->single_v || !tf->single_mlp_gate || !tf->single_mlp_up ||
	    !tf->single_attn_out || !tf->single_concat || !tf->ffn_gate || !tf->ffn_up || !tf->double_img_attn_out || !tf->double_txt_attn_out) {
		tf->work_seq_alloc = 0;
		return -1;
	}
 ...
 */
static void mha_forward(float *out, const float *q, const float *k, const float *v, int seq, int heads, int head_dim, iris_transformer_flux_t *tf) {
	...
	if (iris_metal_available()) {
		...
		iris_metal_attention(out_t, q_t, k_t, v_t, scores, tf->num_heads, seq, seq, head_dim, scale);
 ...
 * Uses pre-allocated workspace buffers from transformer struct
 */
static void joint_attention(float *img_out, float *txt_out, const float *img_q, const float *img_k, const float *img_v, const float *txt_q, const float *txt_k,
                            const float *txt_v, int img_seq, int txt_seq, int heads, int head_dim, iris_transformer_flux_t *tf) {
	int   total_seq = img_seq + txt_seq;
	...
	/* Try fused attention kernel first - operates directly on [seq, hidden] layout
	 * This avoids CPU transpose overhead */
	if (iris_metal_attention_fused(img_out, img_q, cat_k, cat_v, img_seq, total_seq, heads, head_dim, scale) &&
	    iris_metal_attention_fused(txt_out, txt_q, cat_k, cat_v, txt_seq, total_seq, heads, head_dim, scale)) {
		return; /* Success - no transpose needed */
	}
	...
	if (iris_metal_available()) {
		...
		iris_metal_attention(img_out_t, img_q_t, cat_k_t, cat_v_t, scores, heads, img_seq, total_seq, head_dim, scale);
		/* Text attention: txt_Q @ cat_K^T, softmax, @ cat_V */
		iris_metal_attention(txt_out_t, txt_q_t, cat_k_t, cat_v_t, scores, heads, txt_seq, total_seq, head_dim, scale);
	...
	/* CPU fallback: flash attention (memory-efficient, no transpose needed) */
	iris_flash_attention(img_out, img_q, cat_k, cat_v, img_seq, total_seq, heads, head_dim, scale);
	iris_flash_attention(txt_out, txt_q, cat_k, cat_v, txt_seq, total_seq, heads, head_dim, scale);
}
...
static int joint_attention_bf16(iris_gpu_tensor_t img_out, iris_gpu_tensor_t txt_out, iris_gpu_tensor_t img_q, iris_gpu_tensor_t img_k, iris_gpu_tensor_t img_v,
                                iris_gpu_tensor_t txt_q, iris_gpu_tensor_t txt_k, iris_gpu_tensor_t txt_v, int img_seq, int txt_seq, int heads, int head_dim) {
	if (!img_out || !txt_out || !img_q || !img_k || !img_v || !txt_q || !txt_k || !txt_v)
		return 0;
	if (!iris_gpu_tensor_is_f16(img_out) || !iris_gpu_tensor_is_f16(txt_out))
		return 0;
	if (!iris_gpu_tensor_is_f16(img_q) || !iris_gpu_tensor_is_f16(img_k) || !iris_gpu_tensor_is_f16(img_v))
		return 0;
	if (!iris_gpu_tensor_is_f16(txt_q) || !iris_gpu_tensor_is_f16(txt_k) || !iris_gpu_tensor_is_f16(txt_v))
		return 0;
	...
	int ok = 1;
	if (!iris_gpu_attention_fused_bf16(img_out, img_q, cat_k, cat_v, img_seq, total_seq, heads, head_dim, scale)) {
		ok = 0;
	}
	else if (!iris_gpu_attention_fused_bf16(txt_out, txt_q, cat_k, cat_v, txt_seq, total_seq, heads, head_dim, scale)) {
		ok = 0;
	}
...
static int double_block_forward_bf16(iris_gpu_tensor_t img_hidden, iris_gpu_tensor_t txt_hidden, const double_block_t *block, iris_gpu_tensor_t img_shift1,
                                     iris_gpu_tensor_t img_scale1, iris_gpu_tensor_t img_gate1, iris_gpu_tensor_t img_shift2, iris_gpu_tensor_t img_scale2,
                                     iris_gpu_tensor_t img_gate2, iris_gpu_tensor_t txt_shift1, iris_gpu_tensor_t txt_scale1, iris_gpu_tensor_t txt_gate1,
                                     iris_gpu_tensor_t txt_shift2, iris_gpu_tensor_t txt_scale2, iris_gpu_tensor_t txt_gate2, const float *img_rope_cos,
                                     const float *img_rope_sin, const float *txt_rope_cos, const float *txt_rope_sin, int img_seq, int txt_seq,
	...
	if (!block->img_q_weight_bf16 || !block->img_k_weight_bf16 || !block->img_v_weight_bf16 || !block->txt_q_weight_bf16 || !block->txt_k_weight_bf16 ||
	    !block->txt_v_weight_bf16 || !block->img_proj_weight_bf16 || !block->txt_proj_weight_bf16 || !block->img_mlp_gate_weight_bf16 ||
	    !block->img_mlp_up_weight_bf16 || !block->img_mlp_down_weight_bf16 || !block->txt_mlp_gate_weight_bf16 || !block->txt_mlp_up_weight_bf16 ||
		...
		if (!missing_logged) {
			...
			BF16_DEBUG("[BF16] missing bf16 double-block weights:"
			           ...
			           block->img_q_weight_bf16 != NULL, block->img_k_weight_bf16 != NULL, block->img_v_weight_bf16 != NULL,
			           block->img_proj_weight_bf16 != NULL, block->img_mlp_gate_weight_bf16 != NULL, block->img_mlp_up_weight_bf16 != NULL,
			           block->img_mlp_down_weight_bf16 != NULL, block->txt_q_weight_bf16 != NULL, block->txt_k_weight_bf16 != NULL,
			           block->txt_v_weight_bf16 != NULL, block->txt_proj_weight_bf16 != NULL, block->txt_mlp_gate_weight_bf16 != NULL,
			           block->txt_mlp_up_weight_bf16 != NULL, block->txt_mlp_down_weight_bf16 != NULL);
		}
		return 0;
	}
	...
	if (!joint_attention_bf16(img_attn_out, txt_attn_out, img_q, img_k, img_v, txt_q, txt_k, txt_v, img_seq, txt_seq, heads, head_dim)) {
		BF16_DEBUG("[BF16] double block: joint attention failed\n");
		goto cleanup;
	}

	/* Projection + residual */
	img_proj = iris_gpu_linear_bf16_native(img_attn_out, block->img_proj_weight_bf16, img_seq, hidden, hidden);
	txt_proj = iris_gpu_linear_bf16_native(txt_attn_out, block->txt_proj_weight_bf16, txt_seq, hidden, hidden);
	...
	img_gate = iris_gpu_linear_bf16_native(img_norm, block->img_mlp_gate_weight_bf16, img_seq, hidden, mlp_hidden);
	img_up   = iris_gpu_linear_bf16_native(img_norm, block->img_mlp_up_weight_bf16, img_seq, hidden, mlp_hidden);
	txt_gate = iris_gpu_linear_bf16_native(txt_norm, block->txt_mlp_gate_weight_bf16, txt_seq, hidden, mlp_hidden);
	txt_up   = iris_gpu_linear_bf16_native(txt_norm, block->txt_mlp_up_weight_bf16, txt_seq, hidden, mlp_hidden);
	...
	img_down = iris_gpu_linear_bf16_native(img_gate, block->img_mlp_down_weight_bf16, img_seq, mlp_hidden, hidden);
	txt_down = iris_gpu_linear_bf16_native(txt_gate, block->txt_mlp_down_weight_bf16, txt_seq, mlp_hidden, hidden);
...
static void swiglu_ffn_bf16(float *out, const float *x, const float *gate_weight, const float *up_weight, const float *down_weight,
                            const uint16_t *gate_weight_bf16, const uint16_t *up_weight_bf16, const uint16_t *down_weight_bf16, int seq, int hidden,
                            int mlp_hidden, iris_transformer_flux_t *tf) {
...
static void double_block_forward(float *img_hidden, float *txt_hidden, const double_block_t *block, const float *img_mod, const float *txt_mod,
                                 const float *img_rope_cos, const float *img_rope_sin, const float *txt_rope_cos, const float *txt_rope_sin, int img_seq,
                                 int txt_seq, iris_transformer_flux_t *tf) {
	int   hidden     = tf->hidden_size;
	int   heads      = tf->num_heads;
	int   head_dim   = tf->head_dim;
	int   mlp_hidden = tf->mlp_hidden;
	...
	if (block_idx == 0) {
		...
		for (int i = 0; i < 10; i++)
			fprintf(stderr, "%.6f ", img_shift1[i]);
		fprintf(stderr, "\n[DBL] img_mod[0,3072:3082] (scale1): ");
		for (int i = 0; i < 10; i++)
			fprintf(stderr, "%.6f ", img_scale1[i]);
		fprintf(stderr, "\n[DBL] After AdaLN img_norm[0,0,:10]: ");
		for (int i = 0; i < 10; i++)
			fprintf(stderr, "%.6f ", img_norm[i]);
		fprintf(stderr, "\n");
	}
#endif
	...
	iris_gpu_begin_batch();
	LINEAR_BF16_OR_F32(img_q, img_norm, block->img_q_weight, block->img_q_weight_bf16, img_seq, hidden, hidden);
	LINEAR_BF16_OR_F32(img_k, img_norm, block->img_k_weight, block->img_k_weight_bf16, img_seq, hidden, hidden);
	LINEAR_BF16_OR_F32(img_v, img_norm, block->img_v_weight, block->img_v_weight_bf16, img_seq, hidden, hidden);
	iris_gpu_end_batch();

	/* Apply QK normalization (per-head RMSNorm) */
	apply_qk_norm(img_q, img_k, block->img_norm_q_weight, block->img_norm_k_weight, img_seq, heads, head_dim, eps);
	...
	iris_gpu_begin_batch();
	LINEAR_BF16_OR_F32(txt_q, txt_norm, block->txt_q_weight, block->txt_q_weight_bf16, txt_seq, hidden, hidden);
	LINEAR_BF16_OR_F32(txt_k, txt_norm, block->txt_k_weight, block->txt_k_weight_bf16, txt_seq, hidden, hidden);
	LINEAR_BF16_OR_F32(txt_v, txt_norm, block->txt_v_weight, block->txt_v_weight_bf16, txt_seq, hidden, hidden);
	iris_gpu_end_batch();

	/* Apply QK normalization */
	apply_qk_norm(txt_q, txt_k, block->txt_norm_q_weight, block->txt_norm_k_weight, txt_seq, heads, head_dim, eps);
	...
	joint_attention(img_attn_out, txt_attn_out, img_q, img_k, img_v, txt_q, txt_k, txt_v, img_seq, txt_seq, heads, head_dim, tf);

#ifdef DEBUG_DOUBLE_BLOCK
	if (block_idx == 0) {
		fprintf(stderr, "[DBL] After attn img_attn_out[0,0,:5]: ");
		for (int i = 0; i < 5; i++)
			fprintf(stderr, "%.6f ", img_attn_out[i]);
		fprintf(stderr, "\n");
	}
#endif
	...
	iris_gpu_begin_batch();
	LINEAR_BF16_OR_F32(img_proj, img_attn_out, block->img_proj_weight, block->img_proj_weight_bf16, img_seq, hidden, hidden);
	LINEAR_BF16_OR_F32(txt_proj, txt_attn_out, block->txt_proj_weight, block->txt_proj_weight_bf16, txt_seq, hidden, hidden);
	iris_gpu_end_batch();
	...
	if (block_idx == 0) {
		...
		for (int i = 0; i < 5; i++)
			fprintf(stderr, "%.6f ", img_proj[i]);
		fprintf(stderr, "\n");
		fprintf(stderr, "[DBL] gate1[0:5]: ");
		for (int i = 0; i < 5; i++)
			fprintf(stderr, "%.6f ", img_gate1[i]);
		fprintf(stderr, "\n");
	}
#endif
	...
	if (block_idx == 0) {
		...
		for (int i = 0; i < 5; i++)
			fprintf(stderr, "%.6f ", img_hidden[i]);
		fprintf(stderr, "\n");
	}
#endif
	...
	swiglu_ffn_bf16(img_proj, img_norm, block->img_mlp_gate_weight, block->img_mlp_up_weight, block->img_mlp_down_weight, block->img_mlp_gate_weight_bf16,
	                block->img_mlp_up_weight_bf16, block->img_mlp_down_weight_bf16, img_seq, hidden, mlp_hidden, tf);
	...
	swiglu_ffn_bf16(txt_proj, txt_norm, block->txt_mlp_gate_weight, block->txt_mlp_up_weight, block->txt_mlp_down_weight, block->txt_mlp_gate_weight_bf16,
	                block->txt_mlp_up_weight_bf16, block->txt_mlp_down_weight_bf16, txt_seq, hidden, mlp_hidden, tf);
 ...
 */
static int single_block_forward_gpu(float *hidden, const single_block_t *block, const float *t_emb, const float *adaln_weight, const float *img_rope_cos,
                                    const float *img_rope_sin, const float *txt_rope_cos, const float *txt_rope_sin, int seq, int img_offset,
                                    iris_transformer_flux_t *tf) {
	/* Check if GPU tensors are available */
	if (!iris_metal_available() || !iris_metal_shaders_available())
		return 0;
	if (block->qkv_mlp_weight_bf16 == NULL)
		return 0; /* Need bf16 weights */
	...
	if (!norm_gpu || !fused_gpu || !q_gpu || !k_gpu || !v_gpu || !gate_gpu || !up_gpu || !attn_out_gpu || !concat_gpu || !proj_out_gpu) {
	...
	iris_gpu_tensor_t fused_result = iris_gpu_linear_bf16(norm_gpu, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim);
	if (!fused_result) {
		/* Cleanup and fall back */
		iris_gpu_tensor_free(hidden_gpu);
		iris_gpu_tensor_free(norm_gpu);
	...
	iris_gpu_split_qkv_mlp(fused_result, q_gpu, k_gpu, v_gpu, gate_gpu, up_gpu, seq, h_size, mlp_hidden);

	/* === Phase 6: QK RMSNorm on GPU === */
	iris_gpu_qk_rms_norm(q_gpu, k_gpu, block->norm_q_weight, block->norm_k_weight, seq, heads, head_dim, eps);

	/* === Phase 7: Apply unified RoPE on GPU (handles text+image in one call) === */
	iris_gpu_rope_unified(q_gpu, k_gpu, txt_rope_cos, txt_rope_sin, img_rope_cos, img_rope_sin, seq, img_offset, heads, head_dim, axis_dim);

	/* === Phase 8: Self-attention on GPU === */
	/* Try fused attention (seq <= 1024), then bf16 MPS attention (any seq) */
	float attn_scale = 1.0f / sqrtf((float)head_dim);
	if (!iris_gpu_attention_fused(attn_out_gpu, q_gpu, k_gpu, v_gpu, seq, seq, heads, head_dim, attn_scale)) {
		/* Fused attention failed (seq > 1024) - use bf16 MPS attention */
		iris_gpu_batch_end();
		iris_gpu_attention_bf16(attn_out_gpu, q_gpu, k_gpu, v_gpu, seq, seq, heads, head_dim, attn_scale);
		iris_gpu_batch_begin();
	}
	...
	iris_gpu_tensor_free(proj_out_gpu);
	proj_out_gpu = iris_gpu_linear_bf16(concat_gpu, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size);
	if (!proj_out_gpu) {
		...
		iris_linear_nobias_bf16(proj_out_cpu, concat_cpu, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size);
...
static void single_block_gpu_scratch_free(single_block_gpu_scratch_t *s) {
	...
	if (s->q)
		iris_gpu_tensor_free(s->q);
	if (s->k)
		iris_gpu_tensor_free(s->k);
	if (s->v)
		iris_gpu_tensor_free(s->v);
	if (s->gate_mlp)
		iris_gpu_tensor_free(s->gate_mlp);
	if (s->up)
		iris_gpu_tensor_free(s->up);
	if (s->attn_out)
		iris_gpu_tensor_free(s->attn_out);
	if (s->concat)
		iris_gpu_tensor_free(s->concat);
	if (s->proj_out)
		iris_gpu_tensor_free(s->proj_out);
	memset(s, 0, sizeof(*s));
}

static int single_block_gpu_scratch_init(single_block_gpu_scratch_t *s, int seq, int hidden, int mlp_hidden) {
	if (!s || seq <= 0 || hidden <= 0 || mlp_hidden <= 0)
		return 0;
	memset(s, 0, sizeof(*s));
	...
	if (!s->norm || !s->fused || !s->q || !s->k || !s->v || !s->gate_mlp || !s->up || !s->attn_out || !s->concat || !s->proj_out) {
 ...
 * Returns 1 if GPU path was used, 0 to fall back to CPU path.
 */
static int single_block_forward_gpu_chained(iris_gpu_tensor_t hidden_gpu, const single_block_t *block, const float *shift, const float *scale,
                                            const float *gate, const float *img_rope_cos, const float *img_rope_sin, const float *txt_rope_cos,
                                            const float *txt_rope_sin, int seq, int img_offset, iris_transformer_flux_t *tf,
	...
	if (scratch) {
		...
		if (scratch->seq != seq || scratch->hidden != h_size || scratch->mlp_hidden != mlp_hidden || scratch->fused_dim != fused_dim) {
			return 0;
		}
		...
		if (!norm_gpu || !fused_result || !q_gpu || !k_gpu || !v_gpu || !gate_gpu || !up_gpu || !attn_out_gpu || !concat_gpu || !proj_out_gpu) {
			return 0;
		}
	}
	else {
		...
		if (!norm_gpu || !q_gpu || !k_gpu || !v_gpu || !gate_gpu || !up_gpu || !attn_out_gpu || !concat_gpu) {
			/* Cleanup and fall back */
			if (norm_gpu)
				iris_gpu_tensor_free(norm_gpu);
			if (q_gpu)
	...
	/* === Phase 4: Fused QKV + MLP projection on GPU === */
	if (scratch) {
		if (!iris_gpu_linear_bf16_into(fused_result, norm_gpu, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim)) {
			return 0;
		}
	}
	else {
		fused_result = iris_gpu_linear_bf16(norm_gpu, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim);
		if (!fused_result) {
			return 0;
		}
	}
	...
	/* === Phase 11: Final projection on GPU === */
	if (scratch) {
		if (!iris_gpu_linear_bf16_into(proj_out_gpu, concat_gpu, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size)) {
			return 0;
		}
	}
	else {
	...
	if (!own_tensors)
		return 1; /* Using scratch - don't free */
...
static int single_block_bf16_scratch_init(single_block_bf16_scratch_t *s, int seq, int hidden, int mlp_hidden) {
	if (!s || seq <= 0 || hidden <= 0 || mlp_hidden <= 0)
		return 0;
	memset(s, 0, sizeof(*s));
...
static int single_block_forward_bf16(iris_gpu_tensor_t hidden_gpu, const single_block_t *block, iris_gpu_tensor_t shift_bf16, iris_gpu_tensor_t scale_bf16,
                                     iris_gpu_tensor_t gate_bf16, iris_gpu_tensor_t norm_q_weight_bf16, iris_gpu_tensor_t norm_k_weight_bf16,
                                     const float *img_rope_cos, const float *img_rope_sin, const float *txt_rope_cos, const float *txt_rope_sin, int seq,
                                     int img_offset, iris_transformer_flux_t *tf, single_block_bf16_scratch_t *scratch) {
	/* Check if bf16 pipeline is available */
	if (!iris_bf16_pipeline_available())
		return 0;
	if (!hidden_gpu || !iris_gpu_tensor_is_f16(hidden_gpu))
		return 0;
	if (block->qkv_mlp_weight_bf16 == NULL)
		return 0;
	...
	if (scratch) {
		...
		if (!norm_gpu || !fused_result || !q_gpu || !k_gpu || !v_gpu || !gate_mlp_gpu || !up_gpu || !attn_out_gpu || !concat_gpu || !proj_out_gpu) {
			return 0;
		}
	}
	else {
		...
		if (!norm_gpu || !q_gpu || !k_gpu || !v_gpu || !gate_mlp_gpu || !up_gpu || !attn_out_gpu || !concat_gpu) {
			goto cleanup;
		}
	}
	...
	if (scratch) {
		if (!iris_gpu_linear_bf16_native_into(fused_result, norm_gpu, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim)) {
			return 0;
		}
	}
	else {
		fused_result = iris_gpu_linear_bf16_native(norm_gpu, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim);
		if (!fused_result)
			goto cleanup;
	}

	/* === Phase 3: Split fused output (bf16) === */
	iris_gpu_split_qkv_mlp_bf16(fused_result, q_gpu, k_gpu, v_gpu, gate_mlp_gpu, up_gpu, seq, h_size, mlp_hidden);

	/* === Phase 4: QK RMSNorm (bf16) === */
	iris_gpu_qk_rms_norm_bf16(q_gpu, k_gpu, norm_q_weight_bf16, norm_k_weight_bf16, seq, heads, head_dim, eps);

	/* === Phase 5: Apply unified RoPE (bf16 tensors, f32 frequencies) === */
	iris_gpu_rope_unified_bf16(q_gpu, k_gpu, txt_rope_cos, txt_rope_sin, img_rope_cos, img_rope_sin, seq, img_offset, heads, head_dim, axis_dim);

	/* === Phase 6: Self-attention (native bf16 fused SDPA) === */
	float attn_scale = 1.0f / sqrtf((float)head_dim);
	if (!iris_gpu_attention_fused_bf16(attn_out_gpu, q_gpu, k_gpu, v_gpu, seq, seq, heads, head_dim, attn_scale)) {
		goto cleanup;
	}
	...
	if (scratch) {
		if (!iris_gpu_linear_bf16_native_into(proj_out_gpu, concat_gpu, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size)) {
			return 0;
		}
	}
	else {
		proj_out_gpu = iris_gpu_linear_bf16_native(concat_gpu, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size);
		if (!proj_out_gpu)
			goto cleanup;
	}
	...
	if (v_gpu)
		iris_gpu_tensor_free(v_gpu);
	if (gate_mlp_gpu)
		iris_gpu_tensor_free(gate_mlp_gpu);
	if (up_gpu)
		iris_gpu_tensor_free(up_gpu);
	if (attn_out_gpu)
		iris_gpu_tensor_free(attn_out_gpu);
...
static float *iris_transformer_forward_bf16_flux(iris_transformer_flux_t *tf, const float *img_transposed, int img_seq, int extract_seq, const float *txt_emb,
                                                 int txt_seq, const float *t_emb, const float *img_rope_cos, const float *img_rope_sin,
	...
	if (!tf->img_in_weight_bf16 || !tf->txt_in_weight_bf16 || !tf->final_proj_weight_bf16 || !tf->adaln_single_weight || !tf->adaln_double_img_weight ||
	...
	img_hidden = iris_gpu_linear_bf16_native(img_in_bf16, tf->img_in_weight_bf16, img_seq, channels, hidden);
	txt_hidden = iris_gpu_linear_bf16_native(txt_in_bf16, tf->txt_in_weight_bf16, txt_seq, tf->text_dim, hidden);
	...
	if (!img_shift1 || !img_scale1 || !img_gate1 || !img_shift2 || !img_scale2 || !img_gate2 || !txt_shift1 || !txt_scale1 || !txt_gate1 || !txt_shift2 ||
	    !txt_scale2 || !txt_gate2 || !single_shift || !single_scale || !single_gate) {
	...
	for (int i = 0; i < tf->num_double_layers; i++) {
		if (tf->use_mmap) {
			load_double_block_weights(&tf->double_blocks[i], tf->sf_files, tf->num_sf_files, i, tf->hidden_size, tf->mlp_hidden, tf->use_bf16);
		}
		if (!double_block_forward_bf16(img_hidden, txt_hidden, &tf->double_blocks[i], img_shift1, img_scale1, img_gate1, img_shift2, img_scale2, img_gate2,
		                               txt_shift1, txt_scale1, txt_gate1, txt_shift2, txt_scale2, txt_gate2, img_rope_cos, img_rope_sin, txt_rope_cos,
		                               txt_rope_sin, img_seq, txt_seq, tf)) {
	...
	for (int i = 0; i < tf->num_single_layers; i++) {
		if (tf->use_mmap) {
			load_single_block_weights(&tf->single_blocks[i], tf->sf_files, tf->num_sf_files, i, tf->hidden_size, tf->mlp_hidden, tf->use_bf16);
		}
		...
		if (!norm_q || !norm_k ||
		    !single_block_forward_bf16(concat_hidden, &tf->single_blocks[i], single_shift, single_scale, single_gate, norm_q, norm_k, img_rope_cos,
		                               img_rope_sin, txt_rope_cos, txt_rope_sin, total_seq, txt_seq, tf, &single_scratch)) {
			if (norm_q)
	...
	iris_gpu_adaln_norm_bf16(final_norm, img_hidden_final, final_shift, final_scale, extract_seq, hidden, 1e-6f);

	output_bf16 = iris_gpu_linear_bf16_native(final_norm, tf->final_proj_weight_bf16, extract_seq, hidden, channels);
	if (!output_bf16) {
...
cleanup:
	...
	if (txt_in_bf16)
		iris_gpu_tensor_free(txt_in_bf16);
	if (img_hidden)
		iris_gpu_tensor_free(img_hidden);
	if (txt_hidden)
		iris_gpu_tensor_free(txt_hidden);
	if (concat_hidden)
		iris_gpu_tensor_free(concat_hidden);
	if (img_hidden_final)
		iris_gpu_tensor_free(img_hidden_final);
	if (final_norm)
		iris_gpu_tensor_free(final_norm);
	if (output_bf16)
		iris_gpu_tensor_free(output_bf16);
	if (output_f32)
		iris_gpu_tensor_free(output_f32);

	if (img_shift1)
		iris_gpu_tensor_free(img_shift1);
	if (img_scale1)
		iris_gpu_tensor_free(img_scale1);
	if (img_gate1)
		iris_gpu_tensor_free(img_gate1);
	if (img_shift2)
		iris_gpu_tensor_free(img_shift2);
	if (img_scale2)
		iris_gpu_tensor_free(img_scale2);
	if (img_gate2)
		iris_gpu_tensor_free(img_gate2);

	if (txt_shift1)
		iris_gpu_tensor_free(txt_shift1);
	if (txt_scale1)
		iris_gpu_tensor_free(txt_scale1);
	if (txt_gate1)
		iris_gpu_tensor_free(txt_gate1);
	if (txt_shift2)
		iris_gpu_tensor_free(txt_shift2);
	if (txt_scale2)
		iris_gpu_tensor_free(txt_scale2);
	if (txt_gate2)
		iris_gpu_tensor_free(txt_gate2);

	if (single_shift)
		iris_gpu_tensor_free(single_shift);
	if (single_scale)
		iris_gpu_tensor_free(single_scale);
	if (single_gate)
		iris_gpu_tensor_free(single_gate);
	if (final_shift)
		iris_gpu_tensor_free(final_shift);
	if (final_scale)
		iris_gpu_tensor_free(final_scale);
...
static void vkflux_scratch_free(vkflux_scratch_t *s) {
	iris_gpu_tensor_free(s->norm);
	iris_gpu_tensor_free(s->sq);
	iris_gpu_tensor_free(s->sk);
	iris_gpu_tensor_free(s->sv);
	iris_gpu_tensor_free(s->cq);
	iris_gpu_tensor_free(s->ck);
	iris_gpu_tensor_free(s->cv);
	iris_gpu_tensor_free(s->cout);
	iris_gpu_tensor_free(s->proj);
	iris_gpu_tensor_free(s->g0);
	iris_gpu_tensor_free(s->g1);
	iris_gpu_tensor_free(s->fused);
}

static int vkflux_scratch_init(vkflux_scratch_t *s, int total_seq, int hidden, int mlp_hidden, int fused_dim) {
	memset(s, 0, sizeof(*s));
	...
	s->sq    = iris_gpu_tensor_alloc(h);
	s->sk    = iris_gpu_tensor_alloc(h);
	s->sv    = iris_gpu_tensor_alloc(h);
	s->cq    = iris_gpu_tensor_alloc(h);
	s->ck    = iris_gpu_tensor_alloc(h);
	s->cv    = iris_gpu_tensor_alloc(h);
	s->cout  = iris_gpu_tensor_alloc(h);
	s->proj  = iris_gpu_tensor_alloc(h);
	s->g0    = iris_gpu_tensor_alloc(m);
	s->g1    = iris_gpu_tensor_alloc(m);
	s->fused = iris_gpu_tensor_alloc(f);
	if (!s->norm || !s->sq || !s->sk || !s->sv || !s->cq || !s->ck || !s->cv || !s->cout || !s->proj || !s->g0 || !s->g1 || !s->fused) {
...
static vkflux_persist_t *vkflux_persist_get(iris_transformer_flux_t *tf, int img_seq, int txt_seq) {
	...
	if (p &&
	    (p->img_seq != img_seq || p->txt_seq != txt_seq || p->hidden != hidden || p->mlp != mlp || p->channels != channels || p->text_dim != tf->text_dim)) {
	...
	if (!p->scratch_ok || !p->img_in || !p->txt_in || !p->img_hidden || !p->txt_hidden || !p->concat_hidden || !p->final_out) {
		vkflux_persist_free(p);
		return NULL;
	}
	tf->vk_resident = p;
...
static int vkflux_linear(iris_gpu_tensor_t out, iris_gpu_tensor_t x, const float *w_f32, const uint16_t *w_bf16, int seq, int in_dim, int out_dim) {
	if (w_bf16) {
		if (flux_ptr_is_q8(w_bf16))
...
static int double_block_forward_vk(iris_gpu_tensor_t img_hidden, iris_gpu_tensor_t txt_hidden, const double_block_t *b, const float *img_mod,
                                   const float *txt_mod, const float *img_rope_cos, const float *img_rope_sin, const float *txt_rope_cos,
                                   const float *txt_rope_sin, int img_seq, int txt_seq, iris_transformer_flux_t *tf, vkflux_scratch_t *s) {
	int   hidden    = tf->hidden_size;
	int   heads     = tf->num_heads;
	int   head_dim  = tf->head_dim;
	int   mlp       = tf->mlp_hidden;
	...
	if (!vkflux_linear(s->sq, s->norm, b->img_q_weight, b->img_q_weight_bf16, img_seq, hidden, hidden) ||
		...
		return 0;
	iris_gpu_qk_rms_norm(s->sq, s->sk, b->img_norm_q_weight, b->img_norm_k_weight, img_seq, heads, head_dim, eps);
	...
	if (!vkflux_linear(s->sq, s->norm, b->txt_q_weight, b->txt_q_weight_bf16, txt_seq, hidden, hidden) ||
		...
		return 0;
	iris_gpu_qk_rms_norm(s->sq, s->sk, b->txt_norm_q_weight, b->txt_norm_k_weight, txt_seq, heads, head_dim, eps);
	...
	if (!iris_gpu_attention_fused(s->cout, s->cq, s->ck, s->cv, total_seq, total_seq, heads, head_dim, scale))
		return 0;
...
static int single_block_forward_vk(iris_gpu_tensor_t hidden_gpu, const single_block_t *b, const float *single_mod, const float *rope_cos, const float *rope_sin,
                                   int total_seq, iris_transformer_flux_t *tf, vkflux_scratch_t *s) {
	int   hidden    = tf->hidden_size;
	int   heads     = tf->num_heads;
	int   head_dim  = tf->head_dim;
	int   mlp       = tf->mlp_hidden;
	...
	if (!vkflux_linear(s->fused, s->norm, b->qkv_mlp_weight, b->qkv_mlp_weight_bf16, total_seq, hidden, fused_dim))
		return 0;
	...
	iris_gpu_qk_rms_norm(s->cq, s->ck, b->norm_q_weight, b->norm_k_weight, total_seq, heads, head_dim, eps);
	...
	if (!vkflux_linear(s->proj, s->fused, b->proj_mlp_weight, b->proj_mlp_weight_bf16, total_seq, hidden + mlp, hidden))
		return 0;
...
static float *iris_transformer_forward_vulkan_resident_flux(iris_transformer_flux_t *tf, const float *img_transposed, int img_seq, int extract_seq,
	...
	if (tf->use_mmap) {
		for (int i = 0; i < tf->num_double_layers; i++)
			load_double_block_weights(&tf->double_blocks[i], tf->sf_files, tf->num_sf_files, i, hidden, mlp, tf->use_bf16);
		for (int i = 0; i < tf->num_single_layers; i++)
			load_single_block_weights(&tf->single_blocks[i], tf->sf_files, tf->num_sf_files, i, hidden, mlp, tf->use_bf16);
	}
	...
	if (!vkflux_linear(img_hidden, p->img_in, tf->img_in_weight, tf->img_in_weight_bf16, img_seq, channels, hidden) ||
	    !vkflux_linear(txt_hidden, p->txt_in, tf->txt_in_weight, tf->txt_in_weight_bf16, txt_seq, tf->text_dim, hidden))
		goto cleanup;
	...
	for (int i = 0; i < tf->num_double_layers && ok; i++) {
		ok = double_block_forward_vk(img_hidden, txt_hidden, &tf->double_blocks[i], tf->double_mod_img, tf->double_mod_txt, img_rope_cos, img_rope_sin,
		                             txt_rope_cos, txt_rope_sin, img_seq, txt_seq, tf, &scratch);
	...
	if (ok) {
		...
		iris_gpu_copy_region_f32(concat_hidden, (size_t)txt_seq * hidden, img_hidden, 0, (size_t)img_seq * hidden);
	}

	for (int i = 0; i < tf->num_single_layers && ok; i++) {
		ok = single_block_forward_vk(concat_hidden, &tf->single_blocks[i], single_mod, single_rope_cos, single_rope_sin, total_seq, tf, &scratch);
		if (ok && iris_substep_callback)
			iris_substep_callback(IRIS_SUBSTEP_SINGLE_BLOCK, i, tf->num_single_layers);
	}
...
static void single_block_forward(float *hidden, const single_block_t *block, const float *t_emb, const float *adaln_weight, const float *img_rope_cos,
                                 const float *img_rope_sin, const float *txt_rope_cos, const float *txt_rope_sin, int seq, int img_offset,
                                 iris_transformer_flux_t *tf) {
	...
	LINEAR_BF16_OR_F32(fused_out, norm, block->qkv_mlp_weight, block->qkv_mlp_weight_bf16, seq, h_size, fused_dim);
	...
	/* Apply QK normalization */
	apply_qk_norm(q, k, block->norm_q_weight, block->norm_k_weight, seq, heads, head_dim, eps);
	...
	for (int s = 0; s < seq; s++) {
		memcpy(concat + s * (h_size + mlp_hidden), attn_out + s * h_size, h_size * sizeof(float));
		memcpy(concat + s * (h_size + mlp_hidden) + h_size, mlp_gate + s * mlp_hidden, mlp_hidden * sizeof(float));
	}

	float *proj_out = tf->work1;
	LINEAR_BF16_OR_F32(proj_out, concat, block->proj_mlp_weight, block->proj_mlp_weight_bf16, seq, h_size + mlp_hidden, h_size);
...
float *iris_transformer_forward_flux(iris_transformer_flux_t *tf, const float *img_latent, int img_h, int img_w, const float *txt_emb, int txt_seq,
                                     float timestep) {
	int hidden  = tf->hidden_size;
	int img_seq = img_h * img_w;
	...
	if (iris_metal_available() && iris_bf16_pipeline_available() && tf->use_bf16) {
		float *bf16_output = iris_transformer_forward_bf16_flux(tf, img_transposed, img_seq, img_seq, /* extract_seq = img_seq for txt2img */
		                                                        txt_emb, txt_seq, t_emb, img_rope_cos, img_rope_sin, txt_rope_cos, txt_rope_sin);
		if (bf16_output) {
			free(img_transposed);
			free(t_emb);
	...
	LINEAR_BF16_OR_F32(img_hidden, img_transposed, tf->img_in_weight, tf->img_in_weight_bf16, img_seq, tf->latent_channels, hidden);
	free(img_transposed);

	/* Project text embeddings to hidden */
	float *txt_hidden = tf->txt_hidden;
	LINEAR_BF16_OR_F32(txt_hidden, txt_emb, tf->txt_in_weight, tf->txt_in_weight_bf16, txt_seq, tf->text_dim, hidden);

#ifdef DEBUG_TRANSFORMER
	/* Debug: print intermediate values for comparison with Python */
	fprintf(stderr, "\n[DEBUG] t_emb first 10: ");
	for (int i = 0; i < 10; i++)
		fprintf(stderr, "%.6f ", t_emb[i]);
	fprintf(stderr, "\n");
	...
	for (int i = 0; i < 5; i++)
		fprintf(stderr, "%.6f ", txt_hidden[i]);
	fprintf(stderr, "\n");

	fprintf(stderr, "[DEBUG] img_rope_cos[0, :10]: ");
	for (int i = 0; i < 10; i++)
		fprintf(stderr, "%.6f ", img_rope_cos[i]);
	fprintf(stderr, "\n");

	fprintf(stderr, "[DEBUG] txt_rope_cos[10, 96:106]: ");
	for (int i = 96; i < 106; i++)
		fprintf(stderr, "%.6f ", txt_rope_cos[10 * head_dim + i]);
	fprintf(stderr, "\n");
#endif
	...
	iris_linear_nobias(tf->double_mod_img, tf->t_emb_silu, tf->adaln_double_img_weight, 1, hidden, double_mod_size);
	iris_linear_nobias(tf->double_mod_txt, tf->t_emb_silu, tf->adaln_double_txt_weight, 1, hidden, double_mod_size);

	for (int i = 0; i < tf->num_double_layers; i++) {
		/* In mmap mode, load block weights on-demand and free after use */
		if (tf->use_mmap && tf->double_blocks[i].img_q_weight == NULL && tf->double_blocks[i].img_q_weight_bf16 == NULL) {
			load_double_block_weights(&tf->double_blocks[i], tf->sf_files, tf->num_sf_files, i, tf->hidden_size, tf->mlp_hidden, tf->use_bf16);
		}
		double_block_forward(img_hidden, txt_hidden, &tf->double_blocks[i], tf->double_mod_img, tf->double_mod_txt, img_rope_cos, img_rope_sin, txt_rope_cos,
		                     txt_rope_sin, img_seq, txt_seq, tf);
		if (tf->use_mmap)
			free_double_block_weights(&tf->double_blocks[i]);
		if (iris_substep_callback)
		...
		if (i == 0) {
			...
			for (int d = 0; d < 10; d++)
				fprintf(stderr, "%.6f ", img_hidden[d]);
			fprintf(stderr, "\n");
	...
	memcpy(concat_hidden + txt_seq * hidden, img_hidden, img_seq * hidden * sizeof(float));
	...
	if (0 && iris_metal_available() && iris_bf16_pipeline_available() && !tf->use_mmap && tf->use_bf16) {
		...
		if (hidden_f32) {
			...
			if (hidden_bf16) {
				...
				if (shift_f32)
					iris_gpu_tensor_free(shift_f32);
				if (scale_f32)
					iris_gpu_tensor_free(scale_f32);
				if (gate_f32)
					iris_gpu_tensor_free(gate_f32);
				...
				if (shift_bf16 && scale_bf16 && gate_bf16) {
					...
					for (int i = 0; i < tf->num_single_layers && bf16_path_ok; i++) {
						...
						iris_gpu_tensor_t norm_q_f32  = iris_gpu_tensor_create(tf->single_blocks[i].norm_q_weight, tf->head_dim);
						iris_gpu_tensor_t norm_k_f32  = iris_gpu_tensor_create(tf->single_blocks[i].norm_k_weight, tf->head_dim);
						...
						if (norm_q_f32)
							iris_gpu_tensor_free(norm_q_f32);
						if (norm_k_f32)
							iris_gpu_tensor_free(norm_k_f32);

						if (!norm_q_bf16 || !norm_k_bf16 ||
						    !single_block_forward_bf16(hidden_bf16, &tf->single_blocks[i], shift_bf16, scale_bf16, gate_bf16, norm_q_bf16, norm_k_bf16,
						                               img_rope_cos, img_rope_sin, txt_rope_cos, txt_rope_sin, total_seq, txt_seq, tf, NULL)) {
							bf16_path_ok = 0;
							iris_gpu_batch_end();
						...
						if (norm_q_bf16)
							iris_gpu_tensor_free(norm_q_bf16);
						if (norm_k_bf16)
							iris_gpu_tensor_free(norm_k_bf16);
				...
				if (shift_bf16)
					iris_gpu_tensor_free(shift_bf16);
				if (scale_bf16)
					iris_gpu_tensor_free(scale_bf16);
				if (gate_bf16)
					iris_gpu_tensor_free(gate_bf16);
	...
	if (!bf16_path_ok && iris_metal_available() && iris_metal_shaders_available() && !tf->use_mmap) {
		...
		if (concat_hidden_gpu) {
			...
			for (int i = 0; i < tf->num_single_layers && gpu_chained_ok; i++) {
				if (!single_block_forward_gpu_chained(concat_hidden_gpu, &tf->single_blocks[i], precomputed_shift, precomputed_scale, precomputed_gate,
				                                      img_rope_cos, img_rope_sin, txt_rope_cos, txt_rope_sin, total_seq, txt_seq, tf,
			...
			if (scratch_ok)
				single_block_gpu_scratch_free(&scratch);
	...
	if (!bf16_path_ok && !gpu_chained_ok) {
		...
		for (int i = 0; i < tf->num_single_layers; i++) {
			/* In mmap mode, load block weights on-demand and free after use */
			if (tf->use_mmap && tf->single_blocks[i].qkv_mlp_weight == NULL && tf->single_blocks[i].qkv_mlp_weight_bf16 == NULL) {
				load_single_block_weights(&tf->single_blocks[i], tf->sf_files, tf->num_sf_files, i, tf->hidden_size, tf->mlp_hidden, tf->use_bf16);
			}
#ifdef USE_METAL
			/* Try GPU-optimized path first */
			if (!single_block_forward_gpu(concat_hidden, &tf->single_blocks[i], t_emb, tf->adaln_single_weight, img_rope_cos, img_rope_sin, txt_rope_cos,
			                              txt_rope_sin, total_seq, txt_seq, tf))
#endif
			{
				/* Fall back to CPU path */
				single_block_forward(concat_hidden, &tf->single_blocks[i], t_emb, tf->adaln_single_weight, img_rope_cos, img_rope_sin, txt_rope_cos,
				                     txt_rope_sin, total_seq, txt_seq, tf); /* txt_seq is the offset to image */
			}
			if (tf->use_mmap)
				free_single_block_weights(&tf->single_blocks[i]);
			if (iris_substep_callback)
	...
	LINEAR_BF16_OR_F32(output_nlc, final_norm, tf->final_proj_weight, tf->final_proj_weight_bf16, img_seq, hidden, tf->latent_channels);
 ...
 *   Caller must free.
 */
float *iris_transformer_forward_refs_flux(iris_transformer_flux_t *tf, const float *img_latent, int img_h, int img_w, const float *ref_latent, int ref_h,
                                          int ref_w, int t_offset, const float *txt_emb, int txt_seq, float timestep) {
	/* If no reference, delegate to regular function */
	if (ref_latent == NULL) {
		return iris_transformer_forward_flux(tf, img_latent, img_h, img_w, txt_emb, txt_seq, timestep);
	}
	...
	get_cached_combined_rope(tf, img_h, img_w, ref_h, ref_w, t_offset, &combined_rope_cos, &combined_rope_sin);
	...
	if (iris_metal_available() && iris_bf16_pipeline_available() && tf->use_bf16) {
		float *bf16_output = iris_transformer_forward_bf16_flux(tf, combined_transposed, combined_img_seq, img_seq, /* extract_seq = target only */
		                                                        txt_emb, txt_seq, t_emb, combined_rope_cos, combined_rope_sin, txt_rope_cos, txt_rope_sin);
		if (bf16_output) {
	...
	for (int i = 0; i < tf->num_double_layers; i++) {
		...
		double_block_forward(combined_hidden, txt_hidden, &tf->double_blocks[i], tf->double_mod_img, tf->double_mod_txt, combined_rope_cos, combined_rope_sin,
		                     txt_rope_cos, txt_rope_sin, combined_img_seq, txt_seq, tf);
		if (tf->use_mmap) {
			free_double_block_weights(&tf->double_blocks[i]);
	...
	for (int i = 0; i < tf->num_single_layers; i++) {
		...
		single_block_forward(concat_hidden, &tf->single_blocks[i], t_emb, tf->adaln_single_weight, combined_rope_cos, combined_rope_sin, txt_rope_cos,
		                     txt_rope_sin, total_seq, txt_seq, tf);
		if (tf->use_mmap) {
			free_single_block_weights(&tf->single_blocks[i]);
			/* With direct mmap pointers for bf16, no need to clear caches. */
 ...
 */
float *iris_transformer_forward_multirefs_flux(iris_transformer_flux_t *tf, const float *img_latent, int img_h, int img_w, const iris_ref_t *refs, int num_refs,
                                               const float *txt_emb, int txt_seq, float timestep) {
	...
	if (num_refs == 1) {
		return iris_transformer_forward_refs_flux(tf, img_latent, img_h, img_w, refs[0].latent, refs[0].h, refs[0].w, refs[0].t_offset, txt_emb, txt_seq,
		                                          timestep);
	}
	...
	for (int r = 0; r < num_refs; r++) {
		...
		compute_rope_2d_with_t_offset(combined_rope_cos + rope_offset, combined_rope_sin + rope_offset, refs[r].h, refs[r].w, axis_dim, tf->rope_theta,
		                              refs[r].t_offset);
	...
	for (int r = 0; r < num_refs; r++) {
		...
		for (int pos = 0; pos < ref_seq; pos++) {
			for (int c = 0; c < channels; c++) {
				combined_transposed[(trans_offset + pos) * channels + c] = refs[r].latent[c * ref_seq + pos];
			}
		}
...
iris_transformer_flux_t *iris_transformer_load_flux(FILE *f) {
	...
	/* Read config */
	uint32_t config[10];
	if (fread(config, sizeof(uint32_t), 10, f) != 10)
		goto error;
	...
	for (int f = 0; f < num_files; f++) {
		...
		if (t) {
			...
			if (slice >= 0) {
				...
				const uint16_t *src = safetensors_get_bf16_direct(files[f], t) + (size_t)slice * cnt;
				uint16_t       *out = malloc((size_t)cnt * sizeof(uint16_t));
				if (out)
					memcpy(out, src, (size_t)cnt * sizeof(uint16_t));
				return out;
			}
...
iris_transformer_flux_t *iris_transformer_load_safetensors_flux(const char *model_dir) {
	...
	tf->time_embed.fc1_weight = get_sf_tensor_tf(files, num_files, "time_guidance_embed.timestep_embedder.linear_1.weight");
	tf->time_embed.fc2_weight = get_sf_tensor_tf(files, num_files, "time_guidance_embed.timestep_embedder.linear_2.weight");

	/* Modulation weights - these are always needed in f32 for CPU modulation computation */
	tf->adaln_double_img_weight = get_sf_tensor_tf(files, num_files, "double_stream_modulation_img.linear.weight");
	tf->adaln_double_txt_weight = get_sf_tensor_tf(files, num_files, "double_stream_modulation_txt.linear.weight");
	tf->adaln_single_weight     = get_sf_tensor_tf(files, num_files, "single_stream_modulation.linear.weight");
	...
	for (int i = 0; i < tf->num_double_layers; i++) {
		...
		if (tf->use_bf16)
			b->img_q_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->img_q_weight = get_sf_tensor_tf(files, num_files, name);
		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_k.weight", i);
		if (tf->use_bf16)
			b->img_k_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->img_k_weight = get_sf_tensor_tf(files, num_files, name);
		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_v.weight", i);
		if (tf->use_bf16)
			b->img_v_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->img_v_weight = get_sf_tensor_tf(files, num_files, name);

		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_out.0.weight", i);
		if (tf->use_bf16)
			b->img_proj_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->img_proj_weight = get_sf_tensor_tf(files, num_files, name);
		...
		if (tf->use_bf16)
			b->img_mlp_down_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->img_mlp_down_weight = get_sf_tensor_tf(files, num_files, name);
		...
		if (tf->use_bf16)
			b->txt_q_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->txt_q_weight = get_sf_tensor_tf(files, num_files, name);
		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.add_k_proj.weight", i);
		if (tf->use_bf16)
			b->txt_k_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->txt_k_weight = get_sf_tensor_tf(files, num_files, name);
		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.add_v_proj.weight", i);
		if (tf->use_bf16)
			b->txt_v_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->txt_v_weight = get_sf_tensor_tf(files, num_files, name);

		snprintf(name, sizeof(name), "transformer_blocks.%d.attn.to_add_out.weight", i);
		if (tf->use_bf16)
			b->txt_proj_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->txt_proj_weight = get_sf_tensor_tf(files, num_files, name);
		...
		if (tf->use_bf16)
			b->txt_mlp_down_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->txt_mlp_down_weight = get_sf_tensor_tf(files, num_files, name);
	}
	...
	for (int i = 0; i < tf->num_single_layers; i++) {
		...
		if (tf->use_bf16)
			b->qkv_mlp_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->qkv_mlp_weight = get_sf_tensor_tf(files, num_files, name);

		snprintf(name, sizeof(name), "single_transformer_blocks.%d.attn.to_out.weight", i);
		if (tf->use_bf16)
			b->proj_mlp_weight_bf16 = get_sf_tensor_bf16(files, num_files, name);
		else
			b->proj_mlp_weight = get_sf_tensor_tf(files, num_files, name);
	}
...
iris_transformer_flux_t *iris_transformer_load_safetensors_mmap_flux(const char *model_dir) {
	...
	tf->adaln_single_weight     = get_sf_tensor_tf(files, num_files, "single_stream_modulation.linear.weight");
	if (tf->use_bf16) {
		tf->adaln_double_img_weight_bf16 = get_sf_tensor_bf16(files, num_files, "double_stream_modulation_img.linear.weight");
		tf->adaln_double_txt_weight_bf16 = get_sf_tensor_bf16(files, num_files, "double_stream_modulation_txt.linear.weight");
		tf->adaln_single_weight_bf16     = get_sf_tensor_bf16(files, num_files, "single_stream_modulation.linear.weight");
	}
